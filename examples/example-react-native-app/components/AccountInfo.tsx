...
import { Button, Card, Menu, Subheading, Surface, useTheme } from 'react-native-paper';
...
export default function AccountInfo({ accounts, onChange, selectedAccount }: Props) {
    const { colors } = useTheme();
    const selectedAccountPublicKeyBase58String = useMemo(() => selectedAccount.publicKey, [selectedAccount]);
    const selectedAccountLabel = useMemo(() => getLabelFromAccount(selectedAccount), [selectedAccount]);
            ...
            <Card.Content>
                <Suspense fallback={<ActivityIndicator />}>
                    <View style={styles.balanceRow}>
                        ...
                        <FundAccountButton address={selectedAccount.publicKey}>Add Funds</FundAccountButton>
                    </View>
                </Suspense>
                ...
                >
                    <View style={styles.labelRow}>
                        <Icon name="account-balance-wallet" size={18} style={styles.labelIcon} />
                ...
                {accounts.length > 1 ? (
                    <Menu
                        anchor={
                            <Button onPress={() => setMenuVisible(true)} style={styles.addressMenuTrigger}>
                                Change Address
                            </Button>
                        }
                        onDismiss={() => {
