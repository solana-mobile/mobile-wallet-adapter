...
import { VerificationFailed, VerificationState, VerificationSucceeded } from '../utils/ClientTrustUseCase';
...
export default function MWABottomsheetHeader({
    ...
    const iconSource =
        appIdentity?.iconRelativeUri && appIdentity.identityUri
            ? {
                  uri: new URL(appIdentity.iconRelativeUri, appIdentity.identityUri).toString(),
              }
    ...
    return (
        <>
            ...
            <View style={styles.metadataSection}>
                ...
                <Text style={styles.metadataText}>App name: {appIdentity?.identityName}</Text>
                <Text style={styles.metadataText}>App URI: {appIdentity?.identityUri}</Text>
                {verificationState && <Text style={styles.metadataText}>Status: {verificationStatusText()}</Text>}
                {verificationState && (
                    <Text style={styles.metadataText}>Scope: {verificationState?.authorizationScope}</Text>
                )}
            </View>
            <View>{children}</View>
            <Divider style={styles.spacer} />
