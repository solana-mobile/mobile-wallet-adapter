...
import { getCallingPackageUid, verifyCallingPackage } from '@solana-mobile/mobile-wallet-adapter-walletlib';
...
export class ClientTrustUseCase {
    ...
    async verifyPrivaledgedMethodSource(authorizationScope: string, _clientIdentityUri?: string): Promise<boolean> {
        if (authorizationScope.startsWith(AssociationType.LocalFromBrowser)) {
            if (this.associationType !== AssociationType.LocalFromBrowser) {
                ...
                authorizationScope[AssociationType.LocalFromBrowser.length] !== ClientTrustUseCase.SCOPE_DELIMITER
            ) {
                console.warn(
...
abstract class VerificationStateBase {
    ...
    constructor(scopeTag: string, qualifier?: string) {
        ...
        this.authorizationScope =
            qualifier == null ? scopeTag : `${scopeTag}${VerificationStateBase.SCOPE_DELIMITER}${qualifier}`;
    }
}
...
export type VerificationState = VerificationInProgress | VerificationSucceeded | VerificationFailed | NotVerifiable;
