...
function getRequestScreenComponent(request: MWARequest | null | undefined) {
    switch (request?.__type) {
        case MWARequestType.SignAndSendTransactionsRequest:
            return <SignAndSendTransactionsScreen request={request as SignAndSendTransactionsRequest} />;
...
export default function MobileWalletAdapterEntrypointBottomSheet() {
    ...
    const [curRequest, setCurRequest] = useState<MWARequest | undefined>(undefined);
    const [curEvent, setCurEvent] = useState<MWASessionEvent | undefined>(undefined);
    ...
    useEffect(() => {
        async function initializeMWASession() {
            try {
                const sessionId = await initializeMobileWalletAdapterSession('wallet label', config);
    ...
    useEffect(() => {
        const initClientTrustUseCase = async () => {
            ...
            let nextClientTrustUseCase = new ClientTrustUseCase((await Linking.getInitialURL()) ?? '', callingPackage);
            setClientTrustUseCase(nextClientTrustUseCase);
        };
        ...
        >
            <WalletProvider>
                <ClientTrustProvider clientTrustUseCase={clientTrustUseCase}>
                    <View style={styles.bottomSheet}>{getRequestScreenComponent(curRequest)}</View>
                </ClientTrustProvider>
            </WalletProvider>
        </Modal>
    );
