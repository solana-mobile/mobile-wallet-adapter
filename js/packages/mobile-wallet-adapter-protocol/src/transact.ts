...
import { getLocalReflectorAssociateAndroidIntentURL, getRemoteAssociateAndroidIntentURL } from './getAssociateAndroidIntentURL.js';
...
import { launchReflectorAssociation, startSession, startSessionWithReflector } from './startSession.js';
import { 
    AssociationKeypair, 
    MobileWallet, 
    ReflectorWalletAssociationConfig, 
    RemoteMobileWallet, 
    RemoteScenario, 
...
export async function launchAssociation(
    associationUrl: URL
): Promise<void> {
    await launchReflectorAssociation(associationUrl);
}
...
export async function startLocalScenarioOld<TReturn>(
    callback: (wallet: MobileWallet) => TReturn,
    config: ReflectorWalletAssociationConfig,
): Promise<TReturn> {
    assertSecureContext();
    const associationKeypair = await generateAssociationKeypair();
    ...
    return new Promise((resolve, reject) => {
        ...
        const handleMessage = async (evt: MessageEvent<string | Blob>) => {
            ...
            if (encoding == 'base64') { // base64 encoding
                ...
                responseBuffer = toUint8Array(message).buffer;
            } else {
                responseBuffer = await (evt.data as Blob).arrayBuffer();
            }
            switch (state.__type) {
                case 'connecting':
                    if (responseBuffer.byteLength == 0) {
                ...
                case 'reflector_id_received':
                    console.log('message received, state = reflector_id_received, message = ' + new Uint8Array(responseBuffer).toString());
                    if (responseBuffer.byteLength !== 0) {
...
export async function startLocalScenario(
    ...
    const associationUrl = await new Promise<URL>((resolve, reject) => {
        ...
        const handleReflectorIdMessage = async (evt: MessageEvent<string | Blob>) => {
            ...
            if (state.__type === 'connecting') {
                ...
                const associationUrl = await getLocalReflectorAssociateAndroidIntentURL(
                    associationKeypair.publicKey, 
                    config.remoteHostAuthority, 
                    reflectorId,
                    config?.baseUri
