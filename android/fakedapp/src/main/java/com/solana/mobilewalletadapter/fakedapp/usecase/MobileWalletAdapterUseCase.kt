...
object MobileWalletAdapterUseCase {
    ...
    suspend fun authorize(
        client: MobileWalletAdapterClient, identity: DappIdentity, cluster: String?
    ...
    suspend fun reauthorize(
        client: MobileWalletAdapterClient, identity: DappIdentity, currentAuthToken: String
    ): MobileWalletAdapterClient.AuthorizationResult = coroutineScope {
        try {
            runInterruptible(Dispatchers.IO) {
                client.reauthorize(
    ...
    suspend fun deauthorize(
        client: MobileWalletAdapterClient, currentAuthToken: String
    ) = coroutineScope {
        try {
    ...
    suspend fun signTransactions(
        client: MobileWalletAdapterClient, transactions: Array<ByteArray>
        ...
        try {
            ...
            when (val cause = e.cause) {
                ...
                is JsonRpc20Client.JsonRpc20RemoteException -> when (cause.code) {
                    ProtocolContract.ERROR_AUTHORIZATION_FAILED -> throw MobileWalletAdapterOperationFailedException(
                        "Authorization invalid, authorization or reauthorization required", cause
                    )
    ...
    suspend fun signMessages(
        client: MobileWalletAdapterClient, messages: Array<ByteArray>, addresses: Array<ByteArray>
