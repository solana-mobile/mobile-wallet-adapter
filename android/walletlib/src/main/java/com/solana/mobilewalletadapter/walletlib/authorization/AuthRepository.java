...
public class AuthRepository {
    ...
    public synchronized AuthRecord fromAuthToken(@NonNull String authToken) {
        ...
        try (final Cursor c = db.rawQuery("SELECT " +
            ...
            final Uri walletUriBase = c.isNull(6) ?
                    null : Uri.parse(c.getString(6));
            authRecord = new AuthRecord(id, identityRecord, publicKey, scope, walletUriBase,
                    publicKeyId, walletUriBaseId, issued, issued + mAuthIssuerConfig.authorizationValidityMs);
        }
    ...
    public synchronized AuthRecord issue(@NonNull String name,
        ...
        return new AuthRecord(id, identityRecord, publicKey, scope, walletUriBase, publicKeyId,
                walletUriBaseId, now, now + mAuthIssuerConfig.authorizationValidityMs);
    }
